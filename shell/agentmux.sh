...
# session arg of --kill/--suspend/--frame-kill/--probe/attach completes live agentmux session names.
if [ -n "${ZSH_VERSION:-}" ]; then
  _amux_zsh_complete() {
    if (( CURRENT == 2 )); then
      ...
      # Second arg of --kill/--suspend/--frame-kill/--probe/attach is a session name, and
      ...
      case "${words[2]}" in
        @*)
          ...
          compadd -- "${_amux_rp[@]}"
          ;;
        --kill|--suspend|--frame-kill|--probe|attach)
          local -a _amux_sess
          # shellcheck disable=SC2296
