...
- The `--notify` flags in the `PermissionRequest` and `Stop` hooks emit an OSC 777 desktop-notification escape *through the terminal* (a notification-aware host like [warden](https://github.com/lockyc/warden) badges the agent's tab; plain Ghostty raises a system banner; other terminals ignore it) — remove them from `~/.claude/settings.json` if unwanted
- Update agentmux any time with `amux --update` (it's a git clone now — a plain `git pull`).
- Side-terminal layout: `amux --frame [agent] [session]` opens a scratch terminal beside amux (nested tmux, no `~/.tmux.conf` changes). Set the left-pane width with `[frame] left = <percent>`, and optionally split the left column top/bottom with `[frame] left_vertical_split = <percent>` (top region height), with `[frame] left_top_panes = <count>` (`1`–`6`, default `1`) shells stacked in that top region — both built inside the scratch terminal's own session, so `amux --term` (a warden split) shows the same column. Further `[frame]` toggles: `focus = "agent"|"terminal"` (start pane, default agent), `status_position = "bottom"|"top"` (the `[amux frame]` bar, default bottom), and `default = true` to make a bare `amux` open a frame (`amux --no-frame` opts out for one run). Any `[frame]` field can be overridden per-directory (and its subtree) with a `[frame.dirs."<path>"]` block — same matching as an agent's `dirs`, resolved per-field. See the README for the frame's prefix keys.
- Pin a project's **status-bar** colour (distinct from an agent's tab colour) with a per-directory `[amux.dirs."<path>"]` block: `session_colour = "<name>"`. This freezes that project's bar colour and removes it from the auto-assign pool, so no other project ever takes it — even when the pinned project is closed. Without a pin, a session's bar colour is a stable hash of its name, but two projects can collide and swap colours by launch order. Run `amux --colours` for the bar colour names.
- Always-on note row: set `[notes] row = true` in `~/.agentmux/amux.toml` for a fifth status line (tmux's maximum) carrying a per-tab note — click the row, or its `✎ click to add a note` hint, to write or edit it. It is outside the `prefix N` toggle, so it stays visible whether rows 1-3 are showing the AI summary or notes 1-3. Overridable per-directory (and its subtree) with `[notes.dirs."<path>"]`, same matching as `[frame]`'s. Off by default; takes effect the next time you run `amux` in that project.
- Enable a once-daily update check by setting `[update] check = true` in `~/.agentmux/amux.toml`.
- Remote sessions: add a `[[hosts]]` block to `~/.agentmux/amux.toml` (see the example file) to launch/attach sessions on another machine with `amux @<host>`. Auth is entirely ssh's own (`~/.ssh/config` — there's no user/port/key/password field), and everything runs on the remote so closing the laptop never costs you work in flight. See the README's "Remote sessions" section.
