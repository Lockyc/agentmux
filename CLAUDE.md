...
path) — and **none of the three is ever `export`ed** (see the footgun below; a source-level
selftest guard enforces it). So a live-server diagnostic targets *that project's own* socket —
`tmux -L agentmux-<role>-<cksum $PWD>` — not a bare role name.

That `$PWD` is **canonicalised once, on entry to `amux()`** (`_amux_canonicalise_pwd`), and every
`$PWD` read downstream is that value. It is **the single normalisation point for what a project
IS**, because four independent lookups key on the dir and all four compare by exact string
equality: the socket shard above, the restore picker's scope (`dropped --new "$PWD"`), warden's
presence probe, and the `[frame.dirs]` / `[amux.dirs]` / `[notes.dirs]` config tables. The
session ledger keys the *same* project on tmux's `#{pane_current_path}` — i.e. `getcwd()` — so
without this the two spellings of one dir (a symlinked path, or a different case on APFS, which
is case-INsensitive) fork the project in two: a second tmux server for the same directory, and a
restore offer that can never match the ledger rows it is hunting for. Normalise at the entry, not
...
passthrough, extended-keys, sync, hyperlinks — are fully documented at their call sites; see
the pointer list at the end rather than a second copy here.)
...
**Helper libs are LAZY, and the probe path must stay free of them (`bin/amux`, `_amux_need`).** `agent_window_style.sh` (+`colours.sh`), `remote_attach.sh` (+`remote.sh`) and `agentmux-config.sh` are ~2,300 lines that were sourced eagerly on every invocation, each re-resolving its own script dir with forks at source time. Nothing `_amux_probe` needs lives in any of them — it uses only functions defined in `bin/amux` — so warden's per-tab poll was paying for all of it: measured 2026-08-24, the preamble was ~86ms of a ~102ms probe, against ~7ms for the single `tmux` call that actually answers the question. They now load on first use via `_amux_need config|style|remote`, which is idempotent and shares `agentmux-config.sh`'s own `_AMUX_LOADED_CONFIG` guard so the two mechanisms cannot disagree. **The trap is that a missing `_amux_need` fails as wrong behaviour, not as an error**: a config read with the file unsourced returns empty, which reads as "the option is unset" — that is precisely how the `[frame] allow_nested` selftest broke while every other assertion passed. So: put the `_amux_need` line at the top of a *function* that uses a lazy lib, never at the call site, and remember there are **two entry points into this file** — `amux()` (which loads config once at a choke point before dispatch) and the `AMUX_SELFTEST` block (which calls internals like `_amux_frame` directly and therefore loads config itself). Adding a third entry means adding its loads. `--probe` is the one subcommand deliberately exempt from the config choke point; **before exempting another, verify it reads no config the way that exemption was verified** (none of `_amux_default_session_name`, `_amux_{agent,frame}_session_is_mine`, or the socket resolvers calls an `agentmux_*` function), and before adding a config read to anything the probe reaches, don't — that silently re-imposes the 536-line load on the hottest command amux has.

**`sl_prune`'s keep set is the QUERY'S REACHABILITY, and widening it is not the safe direction it looks like (`session_log.sh`).** `sl_dropped` collapses to ONE dead server PER CWD (its "last crash only" stage), so what a query can ever name is: every LIVE server, plus — per cwd — the dead server holding the newest *emittable* row (agent row, resume hint, window in that server's live-set sidecar), plus the same again among the servers the `notified` marker has not yet burned (the gate `--new`/`--pending` read and `--mark` writes, which reaches past the ungated winner). Everything else is dead weight the fold parses on every ledger-path poll; a time-based cutoff cannot see it (measured: 246 servers across 47 cwds, ~200 unreachable). The trap is that the keep set is an **argmax**, so counting a server emittable when it is not does not merely over-retain — it **shadows the real winner and prunes it**. Hence the deliberate asymmetry: when a sidecar cannot be placed on a server (no readable `.sock` companion), prune keeps that server whole AND withholds it from the competition, so neither it nor anything behind it can be lost. Deadness here must be the query's deadness — `_sl_server_live` **paired with the boot epoch**, since a narrower notion of "live" is what lets a wrongly-dead server win a cwd and evict the real answer. Two consequences that must move together: a kept DEAD server is trimmed to the windows in its own (frozen) sidecar — that, not the server count, is what bounds the ledger, since a long-lived project shard logs a row per launch forever while only the windows open AT DEATH are restorable — and the **sidecar sweep runs on the same keep set, matched on `(socket, pid)` from the `.sock` companion**. Matching on the pid alone leaves a same-pid sidecar from a DIFFERENT socket behind, and `_sl_pending_fast` then answers "pending" from a candidate the ledger can no longer see; deleting a sidecar from under a server the ledger still names flips it to the "no sidecar → every window was open at death" reading and re-offers closed tabs. `AGENTMUX_LOG_MAX_LINES` is set against the measured steady state (272 lines on a real 47-project dir), not against "big": a cap far above it just restores the sawtooth the keep set exists to remove.

**The restore offer is consumed by the ANSWER, never by the render — no read may write the `notified` marker (`bin/amux`, `scripts/session_log.sh`).** The gate is once per (dead-server, cwd), and marking it where the rows are *read* is the shape that looks right: one emitter, one mark, obviously "offered once". It is wrong because the read happens **before** the user has answered — so closing the window at the prompt (or Ctrl-C, or a dropped ssh link) discards the offer having done nothing with it. The tabs stay in the ledger, but the only thing that still reaches them is the ungated `amux --restore`, which nothing on screen mentions; the next launch says there is nothing to restore. This already cost a real crashed project its picker. So `--new` (the picker's read) and `--pending` (warden's poll) BOTH gate read-only and mark nothing, and `--mark <cwd>` is the sole writer, called from `_amux_restore_pick` immediately after `read -r reply` returns. Two consequences that must move together: it marks **every cwd that was on screen**, including a decline and the rows left unpicked — the user acted on the *list*, and re-offering the remainder would make `n` unsayable — and it therefore subsumes the per-restored-cwd mark that used to sit in the restore loop, which must not come back as a second writer. A `--pending` read that marks is the same bug wearing warden's clothes: the ghost renders once and never again. Both halves are asserted (`new: does NOT mark notified`, `UNANSWERED offer survives a second read`); **don't remove them.**

**Any new deliberate-teardown path must `session_log.sh discard` the agent shard's sidecar first (`bin/amux`).** A dead server whose sidecar still lists windows *is* the **crash signature** the restore picker recovers from, so an undiscarded kill can come back as an offer to restore. `kill-session` does fire the `window-unlinked` snapshot hook (it empties the sidecar), but asynchronously, racing the server's exit; `kill-server` fires none. That asymmetry is what `--suspend` is built on: it deliberately skips discard and uses `kill-server`, so the shard dies with its live set intact and reads as a crash — never "simplify" it to `kill-session`, which silently makes it an ordinary kill. `_amux_kill`/`_amux_kill_all` write an **empty** sidecar before killing (empty → offer nothing; *absent* → dead legacy server, offer ALL — so discard must write the empty file, never delete it). Add a `--kill`-like command → discard first, or its kills read as crashes. Discard declares that intent **synchronously**; it does not lean on the async `window-unlinked` hook, which covers the ordinary close path (including the last window — see `_sl_live_windows` for the hard case there). (Frame/term kills need none — only agent sessions log `open` events.)

**A path that creates an agent window from a command it already holds must record the resume hint itself — the working hook is not a substitute (`bin/amux`).** `sl_dropped` only ever offers a window whose ledger row carries a non-empty `resume_cmd`, and the agent adapter writes one only when the user actually *submits a prompt* in that tab. So a tab created with a resume command but never prompted in is invisible to the restore picker, and the loss compounds: restore five tabs, work in one, crash, and the next picker offers one — each cycle silently discarding the tabs you hadn't got to yet. `_amux_restore_into` therefore logs `session_log.sh resume` alongside its `open`, passing the window + shard explicitly (the same CLI-path socket rule as the entry below). **`fork_session.sh` is the deliberate exception, and the symmetry is the trap:** a fork's command is `--resume <parent> --fork-session`, so replaying it would fork the *parent* again rather than resume the tab's own session — the fork's real session id doesn't exist until its first prompt, which is precisely when the hook writes it. Recording a hint there would look consistent and restore the wrong session.
...
**Never `export` a per-project socket — the frame publishes them as `@amux_term_socket`/`@amux_agent_socket` session options instead (`bin/amux`, `tmux/frame.conf`, `scripts/frame_reattach.sh`).** Two consumers live *inside* the frame server and need the socket their panes were built with: `frame.conf`'s `C-f Q` (kill the paired terminal) and `frame_reattach.sh` (the pane-died hook). Exporting `AGENTMUX_TERM_SOCKET` looks like the obvious transport — the frame server inherits amux's environment — and it is the trap: an export rides down into **every** descendant of the frame, including a GUI app launched from the scratch terminal, and that app's own later `amux` launches inherit it. `_amux_term_sock` honours an explicit value **verbatim** (the isolation seam), so every project warden opened bound its left pane to the *first* project's term server; two projects sharing a basename (`~/work/api`, `~/personal/api`) then both resolved to `api-term` there and `new-session -A` silently attached the second to the first one's scratch terminal — with the agent pane correct, because the agent socket was never exported. The same asymmetry hid a second bug: `frame_reattach.sh` read `${AGENTMUX_AGENT_SOCKET:-}`, which is *never* set, so after sharding it probed the literal `default` socket and an accidentally-detached agent pane was never reattached. Both are gone now that the sockets travel as session options read back through the dead pane. The export is invisible at runtime (nothing fails; the wrong server is simply *a* working server), so a source-level selftest guard asserts no `export …AGENTMUX_(TERM|FRAME|AGENT)_SOCKET` survives in `bin/amux` — **don't remove it.** Both options are re-asserted on every `amux --frame`, not just the rebuild branch, for the same long-lived-frame-server reason as the hooks and bindings beside them.
...
**Never widen the retry set to "non-zero", and never let the loop start before preflight has succeeded (`scripts/remote.sh`, `scripts/remote_attach.sh`).** `_ra_supervise` re-attaches on a dropped link, and the two ways it can be wrong are not equal: a missed reconnect costs one retyped command, whereas a false retry traps the user in a terminal redialling a session they deliberately left — with the remote session safe either way. So **for ssh** `_rm_classify_exit` retries only on positively-known transport failure (its own 255, SIGHUP/SIGPIPE) and treats **everything ambiguous as exit**; 130/143 are the user asking to stop, and any other non-zero is the REMOTE command's status, which retrying would reproduce forever. **et and mosh are the deliberate exception, and it is literally the widening this rule otherwise forbids** — their default arm retries on *any* non-zero. Both reconnect internally, so reaching us at all means the transport has already given up; neither reserves a status of its own the way ssh reserves 255, so there is no code that separates "the link is gone" from "the remote command failed", and the choice is which error to take. Only 127 (the remote program does not exist) and the shared signal codes are carved out. The accepted cost is real and worth stating: on an `et` host a genuine remote failure loops on the holding screen until the user presses `q`. So the invariant is *never widen **ssh's** set* — a transport that does reserve its own failure code must be classified like ssh, not like et. The second half is the ordering: preflight runs over a non-interactive call **before any tty is allocated**, so an unreachable host, a refused key, a missing project and an uninstalled agentmux all fail where nothing retries. Move the resolution work into the interactive path and every one of those becomes an infinite redial.

**A remote host's liveness is answered ONCE per host, never once per project (`scripts/remote.sh`).** `_rm_roster_json` joins the roster against a single remote `amux --sessions-json`; the join key is the **directory**, because session names are sanitized basenames that collide between same-basename projects and a name join lights the dot on the wrong repo. Probing per project looks equivalent and is a network round trip inside what becomes a poll loop — the exact cost the local presence dot was rebuilt to remove. This is the constraint a future warden remote UI must honour: poll a host once (`--sessions-json` over ssh, joined against the roster), never once per project. Relatedly, the roster **cache** serves tab-completion only (which must be instant and may be stale); the picker and project resolution always go live over the warm master, so a repo cloned a minute ago is launchable before completion has noticed it.

**mosh silently breaks agentmux's escapes, and the ssh ControlPath must keep `%C` (`scripts/remote.sh`).** mosh re-emulates the terminal and forwards only escapes it knows, so OSC 777 notifications, OSC 52 clipboard and tmux passthrough are dropped with nothing reporting an error — `_rm_mosh_warn` states this at use because it is otherwise only discovered. Separately, the ssh ControlPath is an `AF_UNIX` socket under the same 104-char limit the selftests hit with `TMUX_TMPDIR`: `%C` is a fixed-length hash, and "improving" it to the readable `%r@%h:%p` blows the limit on a long alias under a long `$HOME`, breaking multiplexing for that host only.

**Inline images failing in an agent pane is NOT a tmux problem — adding terminal features will not fix it (`docs/multiplexer-direction.md`).** The symptom points at the confs and the adjacent code is `terminal-features`/`allow-passthrough`, so the useless fix is the obvious one. Claude Code's own renderer closes three layers upstream of tmux; passthrough governs images from *other* programs in a pane and never the agent's own output. The mechanism, the closed upstream issues, and the out-of-band shape that does work are in the doc.

**The architectural direction — fewer terminal emulators between the host and each PTY — lives in `docs/multiplexer-direction.md`.** Read it before touching the frame layer, the nesting model, or anything proposing a second session backend; it records what `--frame` is *for* now (amux's compatibility mode, deliberately kept), and why a backend seam comes before any backend.
...
- **OSC 777 notify wraps once *per nested tmux layer*, and `allow-passthrough on` + `extended-keys always` + the `sync` and `hyperlinks` features must be set on *every* socket the escape/keys/redraw/link transit** — `tmux-status.sh` (`_tmux_nest_depth`) and `tmux/{agentmux,frame,term}.conf`, each with full rationale. Under `--frame` the agent is two layers deep, so `bin/amux` re-asserts the socket-level features on the long-lived frame server (which never re-reads its `-f` file). Escape/Ctrl-C dying in a framed agent is `extended-keys off`, not the terminal. An OSC 8 link arriving as inert styled text is the `hyperlinks` feature missing on one of those sockets, not the host: tmux gates re-emission on it and derives it from the terminfo `Hls` cap, which `xterm-ghostty` does not ship — so unlike `sync` (cap present, tmux ignores it) there is nothing for tmux to derive from and naming it is the only route. A feature is computed at **client attach**, so any of these lands on the next reattach, never on the live client.
...
- **Canonicalising `$PWD` needs `env pwd -P`; bash's BUILTIN `pwd -P` silently fixes nothing** — the builtin resolves symlinks *textually* out of `$PWD` and never calls `getcwd()`, so it hands back the typed case unchanged and the shard/ledger mismatch survives looking fixed. `env` is what reaches past the builtin to the real binary — commented at `_amux_canonicalise_pwd` in `bin/amux`.
- **`sl_dropped`'s row ORDER is the restored tab order, and it is not the recency order the same function sorts by** — `_amux_restore_into` creates one window per row *in order* (first row replaces window 0), so emitting the ts-desc stream that last-crash scoping and uuid dedup need restored a crashed session with its tabs reversed. THREE orderings, all load-bearing: recency picks *which* rows survive (and, under `--global`, ranks the projects so the one you were last in leads), window id decides *where* each surviving tab lands *within* its project, and the picker renders recency as a per-row "(ago)" instead. Window ids are unique per SERVER only, so they are comparable inside one project alone — which is why the final sort key is (project rank, window id) and not window id on its own. Read the final-stage comment in `sl_dropped` before reordering any of them for display.
...
- **The remote project picker answers with the chosen row's PATH, and its caller must land it in `RM_PATH`, never `RM_PROJECT`** — a name re-resolves by basename over the host's roots, which two same-basename projects cannot answer, so the picker would offer a choice preflight then rejects as ambiguous — `_ra_pick` in `remote_attach.sh`, consumed in `bin/amux`'s `_amux_remote`.
- **Every path that LANDS the user in a project takes the frame decision through `_amux_use_frame`, never a bare `_amux_attach`** — `amux --restore` attached plainly, so a `[frame] default = true` project got its crashed tabs back in a bare session and the frame never appeared; the two entry points disagreed about the project's own setting — commented at `_amux_use_frame` and at the attach step in `_amux_restore_pick` in `bin/amux`.
- **A `[frame] default = true` launch is decided BEFORE the plain-launch nesting guard, because `--frame` owns the `[frame] allow_nested` hatch and a plain launch deliberately has none** — guarding first makes `amux` and `amux --frame` disagree about a documented escape hatch — commented at the guard in `bin/amux`.
- **A click cannot dismiss the note edit prompt — tmux swallows ALL mouse input while a `command-prompt` is open**, so a `MouseDown1Status` binding to close it installs cleanly and is never invoked; the prompt string names its exits instead — commented at the `command-prompt` call in `scripts/notes.sh`.
...
| `docs/multiplexer-direction.md` | Architecture direction + the external evidence for it (Superlogical's design, transcribed because its sources are agent-unfetchable): why the frame layer stops being warden's path, why `--frame` is kept anyway, why a backend seam precedes any second backend, and why inline images are not a tmux problem — **start here before changing the nesting model or the frame** |
