...
- **bash** (`#!/usr/bin/env bash`) — anything that uses `source`, `local`, `${BASH_SOURCE[0]}`, or arrays. That's `install.sh`, `bin/amux`, `shell/agentmux.sh`, and every config/style consumer (`agentmux-config.sh`, `agent_window_style.sh`, `tab_label.sh`, `cycle_mode.sh`, `launch_agent.sh`, `relaunch.sh`, `fork_session.sh`) — plus `note_popup.sh`, which needs bash for `read -t <n> -s -n 1` (its header says why the POSIX-sh equivalent cannot work).
...
- **A live `display-popup` takes ALL of the client's input, so the edit popup opens BEFORE the prompt and can never be closed by a binding** — with a popup up the command-prompt receives nothing (one shown over it eats the note text), no root-table binding of any kind fires, and nothing reaches the pane; `tmux command-prompt` also blocks its caller, so "open the popup on the next line" runs only once editing is over. Its two exits are therefore its own: the next keystroke, and a TTL backstop — full measurements in `scripts/note_popup.sh`'s header and at the `display-popup` call in `scripts/notes.sh`.
...
NOTE_POPUP_SELFTEST=1        bash scripts/note_popup.sh
...
```bash
bash tests/mouse/run.sh                       # 13 checks; from any cwd
