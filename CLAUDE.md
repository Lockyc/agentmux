...
**The tmux `sync` terminal-feature: what it does and does NOT fix.** `terminal-features 'xterm*:sync,tmux*:sync,screen*:sync'` (set on every socket the redraw transits — `agentmux.conf`/`frame.conf`/`term.conf` for fresh servers, re-asserted in `bin/amux` on every `--frame` for the shared long-lived frame) makes tmux wrap its **full-screen** redraws to ghostty in a DECSET 2026 (synchronized-output) bracket, so ghostty pauses rendering and doesn't tear a full repaint. tmux does **not** derive this from the terminfo `Sync` capability (xterm-ghostty terminfo ships `Sync`, tmux 3.7 still leaves the feature off until named) — so it is worth setting. The feature is computed **at client attach**, so a live frame picks it up on the next reattach, not instantly (`tmux -L agentmux-frame list-clients -F '#{client_termfeatures}'` to check). **BUT this does not fix "the cursor flashes around the screen in an unfocused warden window while Claude thinks."** Verified by capturing the byte stream ghostty receives from the nested stack: tmux emits the 2026 bracket for a *full* redraw but emits **none** for the *small incremental* updates a thinking agent produces (spinner tick + streamed tokens), relaying them as bare cursor-move sequences with the cursor shown. ghostty, which draws the unfocused hollow block cursor on *every* frame (no blink gate — `renderer/cursor.zig`), catches those intermediate positions → the flash. That is an inherent ghostty+tmux limitation (tmux only synchronizes big redraws, ghostty renders faster than the TUI draws), the same class the ghostty maintainers describe as needing end-to-end mode-2026 that tmux doesn't fully provide. Enabling `sync` is still correct (kills full-redraw tearing); it is just **not** the fix for the unfocused-cursor flash, so don't re-attribute that flash to a missing `sync` tag.
