...
Change any one of {`tmux-status.sh` path derivation + its `@amux_runtime_dir` publish, `summary_rows.sh` path derivation, the `#{socket_path}`/`#{@amux_runtime_dir}` args in `agentmux.conf`} and you must change them together.
