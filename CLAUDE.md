...
**Footgun — the summary status rows are a poll, and a too-low `status-interval` makes them a fork bomb.** `agentmux.conf` renders the AI summary via three `status-format[1..3]` lines, each a tmux `#(summary_rows.sh …)`. tmux re-runs every `#()` in the status format **once per `status-interval`, per attached client** — that's the only way tmux shows live content (there is no push). Each `summary_rows.sh` call spawns a few short-lived helpers (`awk`, and coreutils for the socket-hash key). So at `status-interval 1` across many amux panes/clients you spawn on the order of hundreds of processes/second. That's fine while they drain instantly — but there is **no backpressure**: tmux keeps polling regardless. Under any system slowdown (e.g. a Spotlight reindex storm after a hard reboot) the spawns stop draining, accumulate, and exhaust the **per-user process table** (`kern.maxprocperuid`, ~10666) — after which `fork()` fails for everything: new shells hang, commands won't run, apps can't be killed, all while RAM and CPU sit *idle*. It reads like a freeze but is process-table exhaustion (diagnose with `ps -Axo user= | grep -c $(id -un)` climbing past ~400). Confirmed 2026-07-09 (6237 procs / 16475 threads, the list flooded with `bash`/`gawk`/`ggrep`/`uu-coreutils`). Mitigations, in order of leverage: keep `status-interval >= 3` (linearly cuts the spawn rate — it lives in the user's `~/.tmux.conf`, which is sourced *before* and so overrides `agentmux.conf`); minimise spawns per `summary_rows.sh` call (prefer shell built-ins over `printf|tr` etc.); and — the real backpressure fix, not yet shipped — have `summary_rows.sh` skip a tick whose predecessor is still in flight and/or cache its render keyed on the source-file mtime so an unchanged summary re-renders spawn-free. Don't lower `status-interval` back to 1 while these rows are active.
