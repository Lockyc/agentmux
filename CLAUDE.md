...
**Never `export` a per-project socket — the frame publishes them as `@amux_term_socket`/`@amux_agent_socket` session options instead (`bin/amux`, `tmux/frame.conf`, `scripts/frame_reattach.sh`).** Two consumers live *inside* the frame server and need the socket their panes were built with: `frame.conf`'s `C-f Q` (kill the paired terminal) and `frame_reattach.sh` (the pane-died hook). Exporting `AGENTMUX_TERM_SOCKET` looks like the obvious transport — the frame server inherits amux's environment — and it is the trap: an export rides down into **every** descendant of the frame, including a GUI app launched from the scratch terminal, and that app's own later `amux` launches inherit it. `_amux_term_sock` honours an explicit value **verbatim** (the isolation seam), so every project warden opened bound its left pane to the *first* project's term server; two projects sharing a basename (`lachlan/substrate`, `construct/substrate`) then both resolved to `substrate-term` there and `new-session -A` silently attached the second to the first one's scratch terminal — with the agent pane correct, because the agent socket was never exported. The same asymmetry hid a second bug: `frame_reattach.sh` read `${AGENTMUX_AGENT_SOCKET:-}`, which is *never* set, so after sharding it probed the literal `default` socket and an accidentally-detached agent pane was never reattached. Both are gone now that the sockets travel as session options read back through the dead pane. The export is invisible at runtime (nothing fails; the wrong server is simply *a* working server), so a source-level selftest guard asserts no `export …AGENTMUX_(TERM|FRAME|AGENT)_SOCKET` survives in `bin/amux` — **don't remove it.** Both options are re-asserted on every `amux --frame`, not just the rebuild branch, for the same long-lived-frame-server reason as the hooks and bindings beside them.
