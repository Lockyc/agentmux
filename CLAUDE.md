...
## Sockets (the model the footguns assume)
...
**Any new deliberate-teardown path must `session_log.sh discard` the agent shard's sidecar first (`bin/amux`).** The agent session is the only one on its shard, so `kill-session` tears down the whole server — and the `window-unlinked` hook that normally marks a window closed never completes on a dying server, leaving a *populated* sidecar that reads as a **false crash** warden re-offers in the restore picker. `_amux_kill`/`_amux_kill_all` write an **empty** sidecar before killing (empty → offer nothing; *absent* → dead legacy server, offer ALL — so discard must write the empty file, never delete it). Add a `--kill`-like command → discard first, or its kills read as crashes. (Frame/term kills need none — only agent sessions log `open` events.)
