...
**Footgun — a cursor flashing around the screen in an unfocused warden/ghostty window is the tmux `sync` terminal-feature being off, not a warden bug.** ghostty pauses rendering only inside a DECSET 2026 (synchronized-output) bracket; tmux emits that bracket around a redraw ONLY when its `sync` terminal-feature is enabled. Crucially tmux does **not** derive `sync` from the terminfo `Sync` capability — verified: `xterm-ghostty` terminfo ships `Sync`, tmux is 3.7, yet a client's `#{client_termfeatures}` still lacks `sync` until it is named in `terminal-features` (contrast `TEST 1` no-sync vs `TEST 2` with the tag). So warden bundling the ghostty terminfo is *necessary but not sufficient*: without the tag the frame tmux's redraws to the host are unbracketed, ghostty samples half-drawn frames, and an **unfocused** surface repaints its hollow block cursor on every torn frame (a focused one is gated by blink; an unfocused one isn't) — the cursor visibly flashing while a background TUI redraws. Fix, mirroring `extkeys`/`allow-passthrough` exactly: `set -as terminal-features 'xterm*:sync,screen*:sync,tmux*:sync'` on **every socket the redraw transits** — `agentmux.conf`, `frame.conf`, `term.conf` for fresh servers **and** re-asserted in `bin/amux` on every `--frame` for the shared long-lived frame socket. `xterm*` = ghostty (the frame→host hop that actually tears); `tmux*`/`screen*` = the outer frame a nested agent redraws into (`default-terminal` is `tmux-256color`, so `tmux*` is the one that matches today — `screen*` is kept for robustness). Unlike a server option, the feature is computed **at client attach**, so setting it on a live frame does nothing until the next reattach; a running session picks it up on the next `amux --frame`/reattach, not instantly. Check a live server: `tmux -L agentmux-frame list-clients -F '#{client_termfeatures}'` (want `sync` present).
